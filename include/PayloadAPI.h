...
namespace payloadinterpreter {
    ...
    public:
        ...
        virtual void Process(RE::TESObjectREFR* a_holder, const std::string_view& a_payload,
                             RE::BShkbAnimationGraph* a_animationGraph) = 0;
    };
...
/* API INSTRUCTIONS */
//
//	1.	Create a class that inherits from `payloadinterpreter::PayloadHandler' to provide a custom implementation of
// payload handling 		by defining the `Process' function.
//
...
//
//		private:
//			void Process(RE::TESObjectREFR * a_holder, const std::string_view& a_payload, RE::BShkbAnimationGraph*
// a_animationGraph) final
//			{
...
//
//
//	2.	After calling `SKSE::Init' in `SKSEPlugin_Load', register an SKSE messages listener function. In this function,
// register a 		Payload Interpreter messages listener after that plugin is loaded (upon receiving a post load
// message from SKSE). 		If Payload Interpreter is not detected, registering the listener will fail on game launch.
//
//		In `SKSEPlugin_Load':
//
//		...
...
//				if (SKSE::GetMessagingInterface()->RegisterListener("PayloadInterpreter",
// PayloadInterpreterMessageListener)) {
//					// ... success handling
//				} else {
//					// ... failure handling
//				}
...
//				payloadInterpreterMsg->payloadHandlerCollector->RegisterPayloadHandler("Example",
// ExamplePayloadHandler::GetSingleton());
//			}
//		}
